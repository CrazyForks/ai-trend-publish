...
- Cron Triggers 只作为 heartbeat；具体运行时间读取 D1 中的 Dashboard
  schedule，修改定时不需要重新部署。
...
本地、Docker、Cloudflare 和 relay 都继续使用项目内统一的 `Logger`。开启
`observability` 后，所有 `info/warn/error/debug` 日志都会额外镜像到配置的 sink。
workflow 内的日志会自动带上 `runId`、`step`、`profileId`
等字段，便于按一次运行过滤。

常见接入方式：

- Axiom：配置 `observability.axiom.dataset` 和 `observability.axiom.token`。
- Better Stack Logs：配置 `observability.betterStack.sourceToken`。
- 其他日志平台：配置 `observability.http.endpoint`、鉴权头和 `format`。

Cloudflare 仍会在 Workers 日志里看到普通 `console`
输出；外部日志平台用于长期保存、 检索、告警和跨本地/Docker/Cloudflare 聚合。
